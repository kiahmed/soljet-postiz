#!/usr/bin/env bash
# Provision Matrix's posting pipeline on GCP.
#
# Forked from ops/simmer/deploy.sh, NOT a --product flag on it: Matrix does
# not share Simmer's Pub/Sub topic or its poster/snap containers — only the
# two service accounts and the project's Pub/Sub service are shared. See
# docs/matrix_integration.md §Build prerequisites for why this is a fork
# rather than a generalization of the Simmer script.
#
#   ops/matrix/deploy.sh matrix [--sa-only|--snap-only|--poster-only|--pubsub-only|--sub-local|all]
#
# Everything except --sa-only first checks the matrix tier is ENABLED
...
#
# Shared with Simmer (created once, idempotent):
...
#
# Matrix's own (NOT shared with Simmer):
#   Pub/Sub topic   facades.matrix-events
#   Cloud Run   matrix-snap      ops/matrix/snap   (headless-Chromium crop)
#   Cloud Run   matrix-poster    ops/matrix/poster (Pub/Sub push subscriber)
#   Pub/Sub sub matrix-poster-sub   filter attributes.product="matrix",
#               push -> matrix-poster, OIDC as facades-poster-sa
#
# CONFIG: GCP_PROJECT / GCP_REGION / GCP_SA_EMAIL / MATRIX_EVENTS_TOPIC /
...
PRODUCT="${1:?usage: deploy.sh matrix [--sa-only|--snap-only|--poster-only|--pubsub-only|--sub-local|all]}"
MODE="${2:-all}"
...
TOPIC="${MATRIX_EVENTS_TOPIC:-$(envget MATRIX_EVENTS_TOPIC)}"
...
# --- Matrix's own topic + shared runtime SA + its role bindings -----------
ensure_topic(){
...
# --- precondition: the matrix tier must be enabled --------------------
require_tier_ready(){
...
# --- Matrix's own containers --------------------------------------------
deploy_snap(){
  echo "== Cloud Run: $SNAP_SVC =="
  # Screenshots EdgeLane's dedicated render endpoint (GET
  # $MATRIX_API_BASE/matrix/snap/<SYM>?view=<view>, bearer $MATRIX_API_TOKEN),
  # NOT a live login-gated SPA — same lesson simmer-snap learned. See
  # docs/matrix_integration.md §Screenshot capture.
  gc run deploy "$SNAP_SVC" --region="$REGION" \
    --source="$DIR/snap" \
    --no-allow-unauthenticated \
    --service-account="$RUNTIME_SA" \
    --memory=1Gi --cpu=1 --concurrency=1 --timeout=90 \
    --set-env-vars="MATRIX_API_BASE=https://edge.facades.trade" \
    --set-secrets="MATRIX_API_TOKEN=${SECRET_TOKEN}:latest"
...
deploy_poster(){
  ...
  [ -n "$snap_url" ] || { snap_url="https://${SNAP_SVC}-REPLACE-uc.a.run.app"; echo "   ! $SNAP_SVC not deployed — MATRIX_SNAP_URL will need patching"; }
  ...
  gc builds submit "$ROOT" --config="$ROOT/ops/matrix/poster/cloudbuild.yaml" \
...
# rejection, it just multiplies duplicate ERROR posts — see bin/matrix_poster
...
# PULL subscription on Matrix's own topic — for validating against real
# EdgeLane events with `make matrix-poster` before the Cloud Run poster
# exists. Same per-product filter. Idempotent.
deploy_sub_local(){
  ...
  gc pubsub subscriptions create "$sub_local" \
    ...
    || echo "   (exists)"
  # whoever runs `make matrix-poster` locally (GOOGLE_APPLICATION_CREDENTIALS
  ...
  echo
  echo "   drain it:  MATRIX_PUBSUB_SUBSCRIPTION=$sub_local make matrix-poster MODE=draft"
}
