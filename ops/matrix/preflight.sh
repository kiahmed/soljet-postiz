#!/usr/bin/env bash
# Preflight for Matrix's posting pipeline. Forked from ops/simmer/preflight.sh
# (its own topic, not a --product flag on the shared script — see
# docs/matrix_integration.md §Build prerequisites). Checks every dependency
# ops/matrix/deploy.sh needs (and the Cloud Run poster needs at runtime) and
...
PRODUCT="${1:?usage: preflight.sh matrix}"
...
TOPIC="${MATRIX_EVENTS_TOPIC:-$(envget MATRIX_EVENTS_TOPIC)}"; TOPIC="${TOPIC:-facades.matrix-events}"
