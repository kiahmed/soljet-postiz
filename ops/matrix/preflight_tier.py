#!/usr/bin/env python3
"""Tier-config + .env half of ops/matrix/preflight.sh. Exit 1 on any FAIL.

Forked from ops/simmer/preflight_tier.py (MATRIX_* keys instead of SIMMER_*).
"""
import os
import sys
...
for key in (f"POSTIZ_CUSTOMER_ID_{P}", "MATRIX_API_BASE", "MATRIX_API_TOKEN"):
    v = os.environ.get(key, "").strip()
    ok(key, v if key != "MATRIX_API_TOKEN" else v[:6] + "…") if v else bad(key, "unset in .env")

snap = os.environ.get("MATRIX_SNAP_URL", "").strip()
if snap:
    ok("MATRIX_SNAP_URL", snap)
else:
    warn("MATRIX_SNAP_URL", "empty — posts go text-only until the snap service is deployed")
