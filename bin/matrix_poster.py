#!/usr/bin/env python3
"""Matrix (Facades) publisher — the event-driven counterpart to bin/daily.py.

Forked from bin/simmer_poster.py, not a shared multi-tenant poster (see
docs/matrix_integration.md §Build prerequisites for why: each Facades product
gets its own poster/snap containers on its own Pub/Sub topic). The Matrix
engine publishes strategy-grid state-change events to its OWN GCP Pub/Sub
topic (facades.matrix-events, not Simmer's facades.ticker-events; one
subscription per product, filter attributes.product="matrix"). This process
consumes them and posts to the `matrix` tier's channels via the Postiz public
API. It terminates on Cloud Run — no work comes back to this machine; all data
retrieval is over HTTP (EdgeLane read-only API + the matrix-snap screenshot
Cloud Run).

Event shape (Pub/Sub message attributes, body is empty):
    product   "matrix"
    symbol    "NVDA"
    state     pick_selected | bias_aligned | bias_diverged | win_rate_notable |
              session_open | grid_digest | daily_recap
    expiry    "2026-10-02" (or "" — not every state carries one)
    event_id  "MTX-<SYM>-<YYMMDD>-<state>"

As of this writing EdgeLane has not built the Matrix event publisher or
read-only API yet (see docs/matrix_integration.md) — every real event this
poster receives will enrich from a minimal card (recipe_matrix's fallback),
not the full API. That's expected, not a bug: it's the same path Simmer's
poster used before its own API existed.
...
class Dedupe:
    ...
    def __init__(self, tier):
        self.project = (tier.raw.get("MATRIX_PUBSUB_PROJECT")
                        or os.getenv("GCP_PROJECT") or "").strip()
        self.coll = os.getenv("MATRIX_DEDUPE_COLLECTION", "matrix_poster_dedupe")
        self._fs = None
        if self.project and os.getenv("MATRIX_DEDUPE_BACKEND", "auto") != "local":
            try:
...
def process_event(raw_evt: dict, *, tier, dedupe: Dedupe,
    ...
    if not iids:
        ...
        result["reason"] = "integration_ids_for(matrix) is empty (LINKEDIN_ENABLED? X id set?)"
        return result
    ...
    if dry_run:
        ...
        for i in iids:
            ...
            ch_parts, ec = channel_parts(tier, lbl, source_type="matrix_api",
    ...
    for iid in iids:
        ...
        # Per-channel split, not a shared X-sized parts list — compose_matrix()
        # tops out at 260 chars today so this never bit Matrix in practice, but
    ...
    if ok:
        posted_log.mark_posted(source_type="matrix_api", source_id=card_id, tier=TIER_ID,
...
def create_app():
    """gunicorn app factory:  gunicorn 'bin.matrix_poster:create_app()'
    Config comes from the environment (Cloud Run --set-env-vars / --set-secrets):
    MATRIX_POSTER_MODE (default 'now'), MATRIX_POSTER_DRY_RUN."""
    load_dotenv()
    tier = load_tier(TIER_ID)
    dedupe = Dedupe(tier)
    mode = os.environ.get("MATRIX_POSTER_MODE", "now")
    dry_run = os.environ.get("MATRIX_POSTER_DRY_RUN", "").lower() in ("1", "true", "yes")
...
def main() -> int:
    ...
    ap.add_argument("--sub", help="--pull: subscription name override "
                    "(else MATRIX_PUBSUB_SUBSCRIPTION / tier.config)")
